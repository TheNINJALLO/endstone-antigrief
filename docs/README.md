...
# AntiGrief Official Documentation

Welcome to the **AntiGrief for Endstone** official documentation portal. AntiGrief is an advanced, high-performance moderation, audit logging, precise rollback, item recovery, and evidence generation framework designed for Minecraft Bedrock Edition (BDS) servers powered by Endstone.
...
|---|---|---|
| 🚀 **Installation Guide** | Prerequisites, wheel installation, dependency loading, and verification steps. | [Read Guide](installation.md) |
| ⚙️ **Configuration Reference** | Complete `config.json` options, WebUI setup, tuning parameters, and retention policies. | [Read Reference](configuration.md) |
| 📜 **Command Reference** | Comprehensive list of member and operator commands, permissions, and usage examples. | [Read Commands](commands.md) |
| 🌐 **WebUI Security Dashboard** | Web dashboard overview, authentication, live search, NBT inspector, and evidence viewer. | [Read WebUI Guide](webui.md) |
| 🔄 **Rollback & Recovery** | 3-Phase atomic block/metadata/inventory rollback engine & `/agback` workflow. | [Read Rollback Guide](rollback-and-recovery.md) |
| 📦 **Inventories & Bundles** | Container NBT snapshotting, Ender Chest inspection, nested bundles, and UTF-8 safety. | [Read Inventories Guide](inventories-and-bundles.md) |
| 📋 **Grief Evidence Reports** | Immutable SHA-256 evidence hashing, printable PDF reports, and incident verification. | [Read Reports Guide](grief-reports.md) |
| 🗄️ **Database & Maintenance** | SQLite WAL mode schema, automated migration logic, and `/agclean` retention tasks. | [Read Database Guide](database.md) |
| 🩺 **Troubleshooting Matrix** | Diagnostic steps for BlockData connectivity, malformed NBT, and permission issues. | [Read Troubleshooting](troubleshooting.md) |
| 🦖 **Pterodactyl Deployment** | Setup instructions for Pterodactyl Panel containers, environment flags, and egg setup. | [Read Pterodactyl Guide](pterodactyl.md) |
| 📦 **Release & Build Process** | Guide on building wheels locally, running pytest test suites, and GitHub release automation. | [Read Release Guide](release-process.md) |
